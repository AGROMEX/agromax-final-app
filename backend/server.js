...
// Esto permite que las imágenes subidas sean accesibles a través de una URL
...
const storage = multer.diskStorage({
    destination: (req, file, cb) => {
        // Asegúrate de que la carpeta 'uploads' exista en la raíz de tu proyecto
        // Render.com o servicios similares pueden requerir una configuración de almacenamiento en la nube (ej. S3)
        cb(null, 'uploads/'); 
    },
...
// --- MIDDLEWARE DE AUTORIZACIÓN PARA SUPERADMINISTRADOR ---
...
async function startServer() {
    ...
    app.delete('/api/establecimientos/:establecimientoId/vacas/:vacaId', authenticateToken, authorizeEstablecimiento, async (req, res) => {
        const { establecimientoId, vacaId } = req.params;
        try {
            // Se recomienda una "eliminación lógica" (marcar como inactiva) en lugar de física
    ...
    app.post('/api/establecimientos/:establecimientoId/vacas/:vacaId/salud', authenticateToken, authorizeEstablecimiento, async (req, res) => {
        ...
        if (!fecha_evento || !tipo_evento || !descripcion) {
            return res.status(400).json({ message: 'Fecha, tipo de evento y descripción son obligatorios para el registro de salud.' });
        }
        try {
    ...
    app.post('/api/establecimientos/:establecimientoId/vacas/:vacaId/reproduccion', authenticateToken, authorizeEstablecimiento, async (req, res) => {
        ...
        if (!fecha_evento || !tipo_evento) {
            return res.status(400).json({ message: 'Fecha del evento y tipo de evento son obligatorios para el registro de reproducción.' });
        }
        try {
            ...
            res.status(201).json({
                message: 'Registro de reproducción creado exitosamente.',
                registro: result.rows[0]
            });
        } catch (error) {
    ...
    app.post('/api/establecimientos/:establecimientoId/vacas/:vacaId/produccion', authenticateToken, authorizeEstablecimiento, async (req, res) => {
        ...
        if (!fecha_registro || litros_dia === undefined || litros_dia === null) {
            return res.status(400).json({ message: 'Fecha de registro y litros/día son obligatorios para el registro de producción.' });
        }
        try {
    ...
    app.get('/api/establecimientos/:establecimientoId/alertas', authenticateToken, authorizeEstablecimiento, async (req, res) => {
        ...
        try {
            ...
            // Por ahora, generaremos alertas de "Parto Próximo" para vacas preñadas
            // cuya fecha de parto estimada (si se calcula) esté en los próximos 45 días.
            // Para un sistema real, esto implicaría lógica más compleja y quizás una tabla de alertas dedicada.
            ...
            const preñadasQuery = `
                ...
                AND v.estado_reproductivo = 'Preñada'
                AND r.tipo_evento = 'Palpación' -- O el evento que confirme la preñez
            ...
            result.rows.forEach(vaca => {
                // Suponiendo una gestación de 280 días desde la palpación/inseminación
                const fechaReproduccion = new Date(vaca.fecha_reproduccion);
                const fechaPartoEstimada = new Date(fechaReproduccion);
                fechaPartoEstimada.setDate(fechaPartaEstimada.getDate() + 280); // 280 días de gestación
            ...
            // Podrías añadir otras alertas aquí (ej: vacas con tratamientos pendientes, etc.)
    ...
    // --- RUTA DE ADMINISTRACIÓN (ya implementada) ---
